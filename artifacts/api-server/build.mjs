...
async function buildAll() {
  ...
  await esbuild({
    ...
    external: [
      // Externalize express so Vercel can detect it as an Express app
      "express",
      "*.node",
      "sharp",
      "better-sqlite3",
  ...
  // Copy public folder from root (built by Vite) to dist/public
  // so it gets deployed with the serverless function
  try {
    const rootPublicDir = path.resolve(artifactDir, '..', '..', 'public');
    const distPublicDir = path.resolve(distDir, 'public');
    await cp(rootPublicDir, distPublicDir, { recursive: true });
    console.log('Public directory copied to dist/public');
  } catch (err) {
    console.warn('Warning: public directory copy skipped (Vite may not have built yet or directory does not exist)');
  }

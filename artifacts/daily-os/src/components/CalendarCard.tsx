...
export function CalendarCard() {
  ...
  const renderDay = (day: any, isToday: boolean) => (
    <div className={`flex-1 ${isToday ? 'border-r border-white/10 pr-6' : 'pl-6'}`}>
      <h3 className="text-sm font-semibold tracking-wider uppercase text-slate-400 mb-4 flex items-center justify-between">
        ...
        <span className="text-xs text-slate-500">{day ? format(parseISO(day.date), 'MMM d') : ''}</span>
      </h3>
      ...
      {!day || !day.events || day.events.length === 0 ? (
        ...
        <div className="space-y-3">
          {day.events.map((event: any, i: number) => (
            <div key={i} className="group flex gap-3 p-3 rounded-xl bg-slate-800/30 hover:bg-slate-800/60 border border-transparent hover:border-white/5 transition-all">
              <div className="flex flex-col items-end min-w-[60px] text-xs font-mono text-slate-400 mt-0.5">
                ...
                {event.duration && <span className="text-[10px] text-slate-500 opacity-0 group-hover:opacity-100 transition-opacity">{event.duration}</span>}
              </div>
